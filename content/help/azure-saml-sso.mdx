---
title: Configure APLIKASIKITA 
updatedAt: 2025-05-09
summary: Connect Azure Active Directory to APLIKASIKITA so your team signs in with existing corporate credentials.
author: argakuka
categories:
  - analysis
slug: azure-saml-sso
...
<Note variant="success">
  SAML single sign-on is available on APLIKASIKITA Scale and Enterprise plans.
</Note>

APLIKASIKITA Scale customers can enforce single sign-on through Azure Active Directory using the SAML 2.0 protocol. The steps below walk through the Azure configuration and the fields APLIKASIKITA expects.
...
In the next screen, click on **Create your own application**. Give your application a **Name** (e.g. "APLIKASIKITA") and click **Create**.
...
Scroll down to the 3rd section on the page, **SAML Certificates**. Copy the **App Federation Metadata Url** value—APLIKASIKITA uses it to keep your certificate in sync.
...
## Step 5: Configure SAML SSO on APLIKASIKITA

In your APLIKASIKITA workspace, open **Settings → Security**.

<Image
  alt="SAML SSO section on the APLIKASIKITA dashboard"
...
Under **SAML Single Sign-On**, click **Configure**. This opens the APLIKASIKITA SSO modal:
...
Your assigned users should now receive an invitation email to join your APLIKASIKITA workspace.
...
<Note variant="warning">
  ...
  invitation email and be able to join your APLIKASIKITA workspace.
</Note>

They will also be able to sign in to APLIKASIKITA using Azure AD SSO.
